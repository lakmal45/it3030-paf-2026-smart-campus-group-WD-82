...
const UserManagement = () => {
  ...
  // ── Generated-password notification state (success toast) ──
  const [successToast, setSuccessToast] = useState(null); // { email: string }
  ...
  const validateAddForm = () => {
    const errs = {};
    // name is optional — backend derives it from email prefix if blank
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      // Show a simple success toast — password was emailed to the user directly
      setSuccessToast({ email: res.data.email });
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      {successToast &&
        createPortal(
          <div className="fixed bottom-6 right-6 z-[9999] w-full max-w-md animate-slide-in">
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-2xl shadow-2xl shadow-emerald-100/60 p-5">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center flex-shrink-0 text-xl">
                  ✅
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-emerald-700 leading-relaxed">
                    ...
                    <strong>{successToast.email}</strong>. The user can log in
                    with their email and the temporary password from that email.
                  </p>
                  <p className="text-[11px] text-slate-500 mt-2 font-medium">
                    💡 If they have a Gmail address, they can also log in with
                    Google instead.
                  </p>
                </div>
                <button
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* ── Pagination controls ── */}
            {totalPages > 1 && (
              <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-between bg-white rounded-b-2xl">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
              ...
              <div className="space-y-5 mb-8">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. John Doe (leave blank to use email)"
                    className={`w-full px-4 py-3 bg-slate-50/50 border rounded-xl text-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all border-slate-200`}
                  />
                </div>
