...
import { Camera, User, Mail, Shield, Save, Edit2, X, Loader2 } from "lucide-react";
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Your Profile</h1>
          <p className="text-slate-500 mt-1">Manage your account information and how you appear to others.</p>
        </div>
        {!isEditing && (
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-8 flex flex-col items-center">
             <div className="relative group">
                <div className="w-32 h-32 rounded-full border-4 border-slate-50 shadow-inner overflow-hidden bg-slate-100 flex items-center justify-center">
                  {(formData.profileImageUrl || user?.profileImageUrl) ? (
                    <img src={formData.profileImageUrl || user?.profileImageUrl} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
             ...
             <p className="text-xs font-bold text-indigo-500 uppercase tracking-widest mt-1 bg-indigo-50 px-3 py-1 rounded-full">{user?.role?.replace('ROLE_', '') || 'STUDENT'}</p>
             ...
             <div className="w-full mt-8 space-y-4 pt-8 border-t border-slate-50">
                ...
                <div className="flex items-center gap-3 text-slate-600">
                   ...
                   <span className="text-sm font-medium">{user?.role} Permissions</span>
                </div>
             </div>
          </div>
        </div>
              ...
              <div className="p-8 space-y-6">
                 ...
                 <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all disabled:opacity-60 disabled:bg-slate-50/30"
                    />
                    <p className="mt-2 text-xs text-slate-400 font-medium">Provide a direct link to an image file.</p>
                 </div>
